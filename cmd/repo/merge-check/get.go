...
func GetMergeCheckCmd() *cobra.Command {
	var (
	...
	cmd := &cobra.Command{
		Use:   "get",
		Short: "Get ScriptRunner merge checks for repository",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			values, err := client.GetMergeChecks(repositories)
			if err != nil {
				client.Logger.Error(err.Error())
				return nil
			}

			return utils.PrintStructured("repositories", values, output, "projectKey,repositorySlug,mergeChecks.id,mergeChecks.name")
		},
	}
